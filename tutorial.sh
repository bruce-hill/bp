...
for t in $([ $# -gt 0 ] && echo "$@" || ls -v tests/*.sh); do
    ...
    sed -n 's/^# //p' "$t"
    printf "\033[m"
    printf "\033[93;1mGiven these lines:              Give this output:\033[m\n"
    ...
    while true; do
        ...
        printf "\033[93;1mExpected output:                Your pattern's output:\033[m\n"
        bp "$pat" < "${t/.sh/.in}" 2>"$tmpfile" | diff -y -W60 --color=always "${t/.sh/.out}" - && break
        cat "$tmpfile"
        printf "\n\033[0;1;91mSorry, try again!\033[m\n"
    done
    printf "\n\033[0;1;92mCorrect!\033[m\n"
done
